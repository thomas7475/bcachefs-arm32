...
# ---------------------------------------------------------------------------
PATCHES = {
# PATCH 1: Fill _TIME_BITS=64 missing bindgen struct fields
    # Replaces the old 0001-copy_fs patch to explicitly zero-initialize
    # hidden bindgen fields like _bitfield_1.
    ...
    """),
    # PATCH 9: Fill _TIME_BITS=64 missing bindgen struct fields (fusemount)
    # Fixes missing padding fields for c::timespec generated by Bindgen
...
def configure_debian_metadata(work_dir, version_str):
    ...
    if os.path.exists(control_path):
        ...
        content = re.sub(
            ...
            'dkms, rust, rust-src, bindgen', content, flags=re.DOTALL
        )
        # Safely ensure libfuse3-dev is requested in Build-Depends
        if 'libfuse3-dev' not in content:
            content = content.replace('Build-Depends: ', 'Build-Depends: libfuse3-dev, ')
...
def main():
    ...
    # Enable FUSE so Cargo passes --features=fuse
    env["BCACHEFS_FUSE"] = "1"
    
    # 1. Provide the 64-bit time/offset ABI to GCC 
    env["CFLAGS"] = "-Wno-psabi -D_TIME_BITS=64 -D_FILE_OFFSET_BITS=64"
    
    # 2. FORCE Rust's bindgen to explicitly use these Clang arguments when parsing headers.
    #    Without this, bindgen generates 32-bit types, causing struct mismatches!
